...
import { fetchMaintenanceLogs, fetchMaintenanceLog, createMaintenanceLog, completeMaintenance } from '@/api/maintenance';
import type { CreateMaintenanceLogRequest, MaintenanceFilterParams } from '@/types/maintenance.types';
...
export function useCreateMaintenanceLog() {
  ...
  return useMutation({
    mutationFn: (data: CreateMaintenanceLogRequest) => createMaintenanceLog(data),
    onSuccess: () => {
